...
// ----------------------------------------------------------------------------
// Changes made vs. the original file:
// 1. Removed the anti-devtools / right-click / text-selection / console-blocking
//    block. It broke accessibility tools and copy/paste, and the debugger-timing
//    devtools "detector" could freeze the page for legitimate users. None of it
//    provides real protection against someone who actually wants your source.
// 2. Fixed the mandi-price API call: MANDI_API_KEY was referenced but never
//    declared anywhere, so the "real API" branch always threw and silently fell
//    back to mock data. It's now a clearly-labeled constant you fill in, and the
//    function fails loudly (console.warn) instead of pretending to succeed.
// 3. Removed the duplicated databases + duplicated fetchMandiPrices /
//    setAdvisoryMode / showGeneralAdvisory / showSpecificAdvisory that were
//    re-declared inside initVoiceInput(). They silently clobbered the working
//    versions defined earlier in the file and were pure dead weight — same
//    data, second copy, no reason for it to exist.
// 4. Left the two auth flows (basic vs. email-verified) as-is functionally, but
//    flagged clearly below — you likely only want ONE of these wired to your
//    HTML forms, not both. Recommendation and TODO included where they collide.
// 5. Flagged (but did not silently "fix") the plaintext password storage in
...
(function () {
    ...
    // TODO: put your real data.gov.in / AGMARKNET API key here. Previously this
    // constant didn't exist at all, so the "live prices" branch always failed
    // and every request silently used mock data instead.
    const MANDI_API_KEY = '';
    const EMAILJS_SERVICE_ID = 'service_c48sfqj';
    const EMAILJS_PUBLIC_KEY = 'QA0QL3SJlJH2VL0y3';
    // NOTE: the original file used two different template IDs for what looks
    // like two different purposes (signup verification vs. contact form).
    // Keeping both, named clearly, instead of one ambiguous EMAILJS_TEMPLATE_ID.
    ...
    function updateTipOfTheDay() {
        const e = new Date();
        const t = Math.floor((e - new Date(e.getFullYear(), 0, 0)) / (1000 * 60 * 60 * 24)) % farmingTips[currentLanguage].length;
    ...
    // Wire your HTML forms to ONE of these two flows, not both — right now
    // whichever script runs last silently wins if both sets of forms exist.
    // Recommendation: use the "New" (email-verified) flow below and delete
    // this basic one, since it actually checks the email is real.
    //
    // SECURITY NOTE: both flows currently store passwords in plaintext in
    // localStorage. That's fine for a static demo, but it is not safe for any
    // real user data — localStorage is readable by any script on the page and
    // isn't encrypted. Before this goes live with real users, move auth to a
    // real backend with hashed (e.g. bcrypt) passwords, not client-side storage.
...
(function initEmailJSAuth() {
    ...
    async function sendVerificationEmail(email, name, code) {
        try {
            ...
            // Reuses the single EmailJS init from the main IIFE above — no need
            // to call emailjs.init() again here.
...
// NOTE: In the original file this IIFE also contained a full second copy of
// districtDB / crops / advisoryDB / specificCropAdvice / healthyCropsDB plus
// duplicate fetchMandiPrices / setAdvisoryMode / showGeneralAdvisory /
// showSpecificAdvisory functions. Those were byte-for-byte the same data as
// what's already defined and exposed via `window.*` in the main IIFE at the
// top of this file, so they've been removed here — they ran last and were
// silently overwriting the working versions with an identical copy.
(function initVoiceInput() {
  ...
  // Array containing your 100 curated tips
const gardeningTips = [
    // Category 1: Soil & Composting (Days 1–20)
    ...
    // Category 2: Watering & Irrigation (Days 21–40)
    ...
    // Category 3: Planting & Sowing (Days 41–60)
    ...
    // Category 4: Plant Care & Maintenance (Days 61–80)
    ...
    // Category 5: Pest & Disease Management (Days 81–100)
...
function updateWidget() {
    ...
    // 1. Update the live digital clock display
    const timeString = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' });
    document.getElementById('current-time').textContent = timeString;

    // 2. Calculate day of the year to shift tips every 24 hours reliably
    const startOfYear = new Date(now.getFullYear(), 0, 0);
    const diff = now - startOfYear;
    const oneDay = 1000 * 60 * 60 * 24;
    const dayOfYear = Math.floor(diff / oneDay);

    // 3. Find the tip index (loops back to 0 if dayOfYear exceeds array length)
    const tipIndex = dayOfYear % gardeningTips.length;
    
    // 4. Update DOM elements
    const displayNumber = String(tipIndex + 1).padStart(2, '0');
    document.getElementById('tip-number').textContent = `#${displayNumber}`;
    document.getElementById('tip-of-the-day').textContent = gardeningTips[tipIndex];
}

// Run immediately on page load
updateWidget();

// Keep the clock ticking accurately every second
